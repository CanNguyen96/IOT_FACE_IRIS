"""
Fusion evaluation with improved ResNet18 model
Compare: Face + Iris (Simple) vs Face + Iris (ResNet18)
"""
import numpy as np
...
print("="*80)
print("FUSION EVALUATION: Face + Iris (ResNet18)")
print("="*80)
...
# Face (unchanged)
face_data = np.load("../face_cnn/face_embeddings_norm.npz")
face_emb = face_data["embeddings"]
face_lbl = face_data["labels"]
print(f"[OK] Face: {face_emb.shape}")

# Iris ResNet18 (new)
iris_data = np.load("iris_embeddings_resnet18.npz")
...
# Optimized fusion weights from comparison
face_weight = 0.3
iris_weight = 0.7
num_pairs = 3000
random.seed(42)
...
print(f"\n[INFO] Fusion weights: {face_weight:.1f}*face + {iris_weight:.1f}*iris (optimized)")
...
print("="*80)
print(f"Fusion weight: Face={face_weight:.1f}, Iris={iris_weight:.1f}")
...
ax.set_title('ROC Curve - Fusion (Face + Iris ResNet18)')
...
Iris Model: ResNet18 (Pretrained)

Fusion Strategy: Score-level (weighted sum)
Face Weight: {face_weight:.1f}
Iris Weight: {iris_weight:.1f}
...
Test Pairs: {len(genuine_scores) + len(impostor_scores)}
"""
ax.text(0.1, 0.5, info_text, fontsize=10, family='monospace',
        verticalalignment='center')

plt.tight_layout()
plt.savefig('fusion_evaluation_resnet18.png', dpi=150, bbox_inches='tight')
print(f"\n[OK] Saved: fusion_evaluation_resnet18.png")
